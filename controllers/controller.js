...
router.get('/scrape', function(req, res) {
  ...
  request('http://www.theonion.com/', function(error, response, html) {
    ...
    $('article.postlist__item').each(function(i, element) {
        ...
        result.link = 'http://www.theonion.com' + $(this).children('header').children('h1').children('a').attr('href').trim();

        // Collect the Article Summary (contained in the next "div" inside of "this")
         result.summary = $(this).children('.item__content--thumb').children('div').children('p').text().trim() + ""; //convert to string for error handling later
          ...
          if(titlesArray.indexOf(result.title) == -1){
            ...
            Article.count({ title: result.title}, function (err, test){
              ...
              // Log that scrape is working, just the content was already in the Database
                ...
                console.log('Extra, Not saved to DB.')
              }
